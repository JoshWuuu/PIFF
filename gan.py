...
def train():
    ...
    # --- Folders for results ---
    os.makedirs("gan_sp_outputs/images", exist_ok=True)
    os.makedirs("gan_sp_outputs/models", exist_ok=True)
    ...
    for epoch in range(epochs):
        for i, (flood, dem, rainfall, spm) in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}")):
            ...
            fake_target_for_D = generator(dem, step, rainfall_embedding, spm=spm).detach()
        ...
        # --- Logging and Saving ---
        print(f"\n[Epoch {epoch+1}/{epochs}] [D loss: {loss_D.item():.4f}] [G loss: {loss_G.item():.4f}, adv: {loss_G_adv.item():.4f}, l1: {loss_G_l1.item():.4f}]")
        ...
        vutils.save_image(sample_imgs, f"gan_sp_outputs/images/{epoch+1:03d}.png", nrow=2, normalize=False)

    # Save final model
    torch.save(generator.state_dict(), "gan_sp_outputs/models/generator.pth")
    torch.save(rainfall_emb.state_dict(), "gan_sp_outputs/models/rainfall_emb.pth")
    print("--- Training Finished. Model saved. ---")


def inference():
    print("\n--- Starting Inference ---")
    os.makedirs("gan_sp_outputs/test_images", exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    noise_level = torch.linspace(1, 0, steps=1000).to(device)
    generator = Image256Net(log, noise_levels=noise_level).to(device)
    # generator = UnetGenerator(in_channels=condition_channels, out_channels=target_channels).to(device)

    generator.load_state_dict(torch.load("gan_sp_outputs/models/generator.pth"))
    generator.eval()
    rainfall_emb = RainfallEmbedder(256, 1).to(device)
    rainfall_emb.load_state_dict(torch.load("gan_sp_outputs/models/rainfall_emb.pth"))
    rainfall_emb.eval()
    ...
    with torch.no_grad():
        for i, (flood, dem, rainfall, spm) in enumerate(tqdm(test_dataloader, desc="Generating Images")):
            ...
            for j in range(flood.size(0)):
                ...
                vutils.save_image(sample_imgs, f"gan_sp_outputs/test_images/{file_index:04d}.png")
                file_index += 1
